...
async function apiCall(action, params={}){
  ...
  if(!j.ok){
    if(String(j.error||'').indexOf(ERR_SESION)>=0){ sesionVencida(); throw new Error('Tu sesión se cerró por seguridad. Vuelve a entrar con tu PIN.'); }
...
const Solicitudes = {
  ...
  async cargar(){
    ...
    this.pagina=1;
    try{
      const r=await apiGet('getSolicitudes', this.params(1));
