...
const Solicitudes = {
  ...
  async cargar(){
    ...
    this.pagina=1;
    try{
      const r=await apiGet('getSolicitudes',{estado:this.filtroEstado, q:this.filtroTexto, pagina:1, porPagina:this.porPagina});
  ...
  async cargarMas(){
    ...
    try{
      const r=await apiGet('getSolicitudes',{estado:this.filtroEstado, q:this.filtroTexto, pagina:this.pagina+1, porPagina:this.porPagina});
      this.aplicar(r, true);
      this.render();
    }catch(e){ alertErr('Error al cargar',e.message); }
  ...
  render(){
    ...
    if(sub) sub.textContent = this.total>this.data.length
      ? `${this.data.length} de ${this.total} solicitudes`
    ...
    if(this.hayMas){
      cont.insertAdjacentHTML('beforeend',
        `<button id="sol-mas" class="btn btn-ghost btn-block mt-sm">Ver más (${this.total-this.data.length} restantes)</button>`);
