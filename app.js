...
const API_LECTURAS = ['getsolicitudes','getsolicitud','getprofesionales','getcatalogos',
...
function apiEspejoGuardar_(clave, dato){
  if(!clave) return;
  try{
    localStorage.setItem(clave, JSON.stringify({t:Date.now(), d:dato}));
    apiEspejoPodar_();
...
function logout(){ Bot.detenerPollingQR(); apiEspejoLimpiar_(); localStorage.removeItem(SESSION_KEY); state.user=null; state.token=''; state.pinBuffer=''; pintarPin(); showView('login'); }
...
const Solicitudes = {
  ...
  /* FASE 2: la lista viene por páginas. Antes el servidor mandaba las
     1.558 filas completas en cada carga y el teléfono las pintaba todas. */
  pagina:1, porPagina:30, total:0, hayMas:false, conteos:{}, cargandoMas:false,
  /* FASE 3: filtros nuevos (todos los resuelve el servidor, por eso
     siguen funcionando con la lista paginada). */
  filtroProfesional:'', filtroOrigen:'', orden:'RECIENTES', profesionalesEnUso:[],

  /* FASE 8: abrir() ya no hace tres viajes seguidos. Lo que falte
     (profesionales, catálogos) viaja JUNTO con la lista en una sola
     llamada. Se mantiene la llamada a cargar() porque la capa de
     esqueletos la envuelve: si abriéramos por otro camino, las
     siluetas de carga dejarían de salir. */
  async abrir(){
  ...
  async cargar(){
    ...
    try{
      ...
      let r;
      if(extras.length){
        const lista=extras.map(a=>({a:a})).concat([{a:'getSolicitudes', p:this.params(1)}]);
        const res=await apiLote(lista);
        extras.forEach((a,i)=>{
          const v=res[i];
          if(v instanceof Error) return;                 // no es crítico: se reintenta al volver a abrir
        ...
        r=await apiGet('getSolicitudes', this.params(1));
      }
      this.aplicar(r, false);
      this.render();
    }catch(e){ alertErr('Error al cargar',e.message); }
    finally{ stopLoading(); }
  },
  /* Nombre distinto a cargar() a propósito: la capa de esqueletos
     envuelve cargar() y repintaría siluetas encima de lo ya cargado. */
  async cargarMas(){
    if(this.cargandoMas || !this.hayMas) return;
    this.cargandoMas=true;
    const btn=$('#sol-mas'); if(btn){ btn.disabled=true; btn.textContent='Cargando…'; }
    try{
      const r=await apiGet('getSolicitudes', this.params(this.pagina+1));
      this.aplicar(r, true);
      this.render();
    }catch(e){ alertErr('Error al cargar',e.message); }
    finally{ this.cargandoMas=false; }
  },
  /* La barra de filtros se pega justo debajo del encabezado. La altura
     del encabezado cambia con el tamaño de letra del teléfono, así que
     se mide en vez de darla por sentada. */
  medirEncabezado(){
  ...
  params(pagina){
    return {
      ...
      orden:this.orden, pagina:pagina, porPagina:this.porPagina
    };
  },
  hayFiltros(){
  ...
  aplicar(r, agregar){
    ...
    this.data = agregar ? this.data.concat(items) : items;
    this.total = (r && r.total!=null) ? r.total : this.data.length;
    this.pagina = (r && r.pagina) ? r.pagina : 1;
    this.hayMas = !!(r && r.hayMas);
    this.conteos = (r && r.conteos) ? r.conteos : {};
    if(r && r.profesionales) this.profesionalesEnUso=r.profesionales;
  },

  /* Los chips se pintan con su conteo, y el conteo viene ya filtrado
     por búsqueda/profesional/origen: el número dice cuántas verías si
     tocaras ese chip, no cuántas hay en toda la base. */
  pintarChips(){
    ...
    $$('.chip',cont).forEach(c=>c.addEventListener('click',()=>{
      ...
      this.filtroEstado=c.dataset.estado; this.cargar();
    }));
  },
  ...
  setupListeners(){
    ...
    /* Los chips ya no viven en el HTML: los pinta render() con su
       conteo, y ahí mismo quedan enganchados. */
    ...
    if(sInp){
      ...
      sInp.addEventListener('input',()=>{
        ...
        t=setTimeout(()=>{
          ...
          this.filtroTexto=v; this.cargar();
        },350);
      });
    }
    if(sClr) sClr.addEventListener('click',()=>{
      sInp.value=''; sWrap.classList.remove('tiene-texto');
      if(this.filtroTexto){ this.filtroTexto=''; this.cargar(); }
      sInp.focus();
    });

    const ord=$('#sol-orden');
    if(ord) ord.addEventListener('change',()=>{
      this.orden=ord.value; ord.classList.toggle('activo', this.orden!=='RECIENTES'); this.cargar();
    });
    const prof=$('#sol-profesional');
    if(prof) prof.addEventListener('change',()=>{
      this.filtroProfesional=prof.value; prof.classList.toggle('activo', !!prof.value); this.cargar();
    });
    const org=$('#sol-origen');
    if(org) org.addEventListener('change',()=>{
      this.filtroOrigen=org.value; org.classList.toggle('activo', !!org.value); this.cargar();
    });
...
const Pacientes = {
  data:[], filtro:'',
  pagina:1, porPagina:30, total:0, hayMas:false, cargandoMas:false,
  async abrir(){ showView('pacientes'); await this.cargar(); },
  async cargar(){
    startLoading();
    this.pagina=1;
    try{
      const r=await apiGet('getPacientes',{q:this.filtro, pagina:1, porPagina:this.porPagina});
      this.aplicar(r,false); this.render();
    }
    catch(e){ alertErr('Error',e.message); }
    finally{ stopLoading(); }
  },
  async cargarMas(){
    if(this.cargandoMas || !this.hayMas) return;
    this.cargandoMas=true;
    const btn=$('#pac-mas'); if(btn){ btn.disabled=true; btn.textContent='Cargando…'; }
    try{
      const r=await apiGet('getPacientes',{q:this.filtro, pagina:this.pagina+1, porPagina:this.porPagina});
      this.aplicar(r,true); this.render();
    }catch(e){ alertErr('Error',e.message); }
    finally{ this.cargandoMas=false; }
  },
  aplicar(r, agregar){
