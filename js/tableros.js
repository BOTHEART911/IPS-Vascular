 ...
 *   Del servidor (Tablero.gs). El de Solicitudes viaja DENTRO de
 *   la respuesta de getSolicitudes, así que no cuesta una llamada
 *   extra y no puede discrepar de la lista que se está viendo.
 *
 * CÓMO SE ENGANCHA
  ...
  function enganchar() {
    if (typeof Solicitudes !== 'undefined') {
      ...
      Solicitudes.params = function (pagina) {
        ...
        if (pagina === 1) o.tablero = '1';      // solo la primera página trae tablero
        return o;
      };
      var aplicar = Solicitudes.aplicar;
      Solicitudes.aplicar = function (r, agregar) {
        if (r && r.tablero) DATOS.solicitudes = r.tablero;
        return aplicar.call(this, r, agregar);
      };
      var render = Solicitudes.render;
      Solicitudes.render = function () {
        var v = render.call(this);
        try { pintarSolicitudes(); } catch (e) {}
        return v;
      };
    }
    ...
    if (typeof Profesionales !== 'undefined') {
      ...
      Profesionales.cargar = async function () {
        ...
        try { DATOS.profesionales = await apiGet('getTableroProfesionales'); } catch (e) {}
        this.render();
      };
    ...
    if (typeof Pacientes !== 'undefined') {
      ...
      Pacientes.abrir = async function () {
        ...
        if (!DATOS.pacientes) {
          try { DATOS.pacientes = await apiGet('getTableroPacientes'); } catch (e) {}
